...
import static com.example.boil1.Main.format;
...
public class CriticalPath {
    ...
    public static Collection print(Task[] tasks) {
        Collection<String> list = new ArrayList<>();
        String column_name = String.format("%-20s %-10s %-10s %-10s %-10s %-10s %-15s\n", "Task", "ES", "EF", "LS", "LF", "Slack", "Critical");
        String text = "";
        text += column_name;
        list.add(column_name);
        String[] summary = null;
        for (Task t : tasks) {
            text += t.toStringArray();
            text += "\n";
